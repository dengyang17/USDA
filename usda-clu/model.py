...
class LatentClustering(nn.Module):
    def __init__(self, input_size, latent_size, hidden_size, num_cluster):
        super().__init__()
        self.encoder = MLP(input_size, latent_size, hidden_size)
        self.decoder = MLP(latent_size, input_size, hidden_size)
        self.loss = nn.MSELoss()
        self.Z = nn.Parameter(data=torch.Tensor(num_cluster, latent_size))
    ...
    def forward(self, X):
        ...
        M_cluster = H.mm(self.Z.permute(1,0))
        p_cluster = F.softmax(M_cluster, dim=-1)
        H_ = p_cluster.mm(self.Z)

        output = self.decoder(H_)
        reconstruct_loss = self.loss(X, output)
        self_repre_loss = self.loss(H, H_)

        M_cluster = M_cluster.view(s1, s2, -1)
        output = output.view(s1, s2, -1)
        H_ = H_.view(s1, s2, -1)

        act_loss = (reconstruct_loss, self_repre_loss, self._ortho_regularizer())

        return output, M_cluster, act_loss

    def _ortho_regularizer(self):
        return torch.norm(torch.matmul(self.Z, self.Z.t()) - torch.eye(self.num_cluster).to(self.Z.device))
...
class USDA(nn.Module):
    def __init__(self, args, vocab_size, class_num, pretrained_private=None, pretrained_encoder=None):
        ...
        self.gru = nn.GRU(d_model+self.class_num, d_model, num_layers=1, bidirectional=False, batch_first=True)

        #self.U = nn.Parameter(torch.Tensor(d_model, d_model))

        #self.act_classifier = nn.Linear(d_model, class_num)
        self.act_cluster = LatentClustering(d_model, d_model//2, d_model//4, args.class_num)
    ...
    def forward(self, input_ids, act_seq=None, sat=None, **kwargs):
        ...
        _, act_res, act_loss = self.act_cluster(H)

        H = torch.cat([H, act_res], dim=-1)
        _, hidden = self.gru(H)
        hidden = hidden.squeeze(0)

        '''
        att = torch.bmm(torch.matmul(H, self.U), hidden.permute(1,2,0))
        att = F.softmax(torch.tanh(att), dim=1)
        hidden = torch.bmm(H.permute(0,2,1), att).squeeze(-1)
        '''
...
class USDA_A(nn.Module):
    ...
    def forward(self, input_ids, act_seq=None, sat=None, **kwargs):
        ...
        att = self.w(torch.tanh(self.U(H))).squeeze(-1)
        att = F.softmax(att.masked_fill(mask=~attention_mask, value=-np.inf), dim=1).unsqueeze(-1)
        hidden = torch.bmm(H.permute(0,2,1), att).squeeze(-1)
...
class USDA_P(nn.Module):
    def __init__(self, args, vocab_size, class_num, pretrained_private=None, pretrained_encoder=None):
        ...
        #self.act_classifier = nn.Linear(d_model, class_num)
        self.act_cluster = LatentClustering(d_model, d_model//2, d_model//4, args.class_num)
        #self.sat_classifier = MLP(d_model, 3, d_model//2)
        self.sat_classifier = nn.Linear(d_model, 3)
    ...
    def forward(self, input_ids, act_seq=None, sat=None, **kwargs):
        ...
        #content_hidden = content_hidden.squeeze(0)
